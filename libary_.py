...
def creating_study_periods(frequencies, frequencies_number_of_samples, frequency_index):
    ...
    for i in range(number_of_study_periods):
        study_periods[0,i] = Returns[i*frequencies_number_of_samples[frequency_index]/2:\
                                        (i+4)*frequencies_number_of_samples[frequency_index]/2].flatten()
        study_periods[1,i] = dates.iloc[i*frequencies_number_of_samples[frequency_index]/2:\
                                        (i+4)*frequencies_number_of_samples[frequency_index]/2].to_numpy().flatten()
