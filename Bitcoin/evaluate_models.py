...
def calculate_da_mse(model_names, frequencies, number_of_study_periods, study_periods):
    ...
    plt.bar(np.arange(20)-0.32, data[:,0], 0.16, label='ARMA')
    plt.bar(np.arange(20)-0.16, data[:,1], 0.16, label='LSTM')
    plt.bar(np.arange(20), data[:,2], 0.16, label='GRU')
    plt.bar(np.arange(20)+0.16, data[:,3], 0.16, label='RF')
    plt.bar(np.arange(20)+0.32, data[:,4], 0.16, label='ENS')
    plt.xticks(np.arange(20), ['2017-11', '2017-12', '2018-01', '2018-02', '2018-03', '2018-04', \
                               '2018-05', '2018-06', '2018-07', '2018-08', '2018-09', '2018-10', \
                               '2018-11', '2018-12', '2019-01', '2019-02', '2019-03', '2019-04', \
                               '2019-05', '2019-06'], rotation=30, ha='right')
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.15), ncol=5)
    plt.tight_layout()
    plt.savefig('figures/MSE_over_study_periods_bc.png')
    plt.show()
    ...
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.15), ncol=5)
    plt.tight_layout()
    plt.savefig('figures/Directional_accuracy_over_study_periods_bc.png')
    plt.show()
    ...
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.15), ncol=5)
    plt.tight_layout()
    plt.savefig('figures/MSE_over_frequencies_bc.png')
    plt.show()
    ...
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.15), ncol=5)
    plt.tight_layout()
    plt.savefig('figures/Directional_accuracy_over_frequencies_bc.png')
    plt.show()
...
def vis_cum_logr(logr, returns, trading_strategy, frequencies, dates, number_of_study_periods, transaction_cost, model_names):
    ...
    plt.savefig('figures/Cumulative_logreturns_'+str(transaction_cost).replace('.','')+'_bc.png')
...
def calculate_pred_corr(predictions, returns, model_names):
    ...
    for frequency_index in range(5):
        ...
        plt.savefig('figures/pred_corr_matrix_frequency_'+str(frequency_index)+'_bc.png')
        plt.show()
    ...
    for frequency_index in range(5):
        ...
        for i in range(len(corr_labels)):
            for j in range(len(corr_labels)):
                text = ax.text(j, i, np.round(corr_matrix[i, j],2), ha="center", va="center", color="black")
        ax.set_ylim(len(corr_labels)-0.5, -0.5)
        plt.savefig('figures/error_corr_matrix_frequency_'+str(frequency_index)+'_bc.png')
        plt.show()
